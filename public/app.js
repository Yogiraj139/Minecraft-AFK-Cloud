(function () {
    ...
    for (const proxy of state.proxies) {
      ...
      row.innerHTML = `
        ...
        <div class="proxy-actions">
          <button class="btn btn-secondary" type="button" data-test-proxy="${proxy.id}">Test</button>
  ...
  document.getElementById('proxyFileInput').addEventListener('change', async (event) => {
    ...
    document.getElementById('proxyImportForm').elements.proxies.value = text;
  ...
  proxyList.addEventListener('click', async (event) => {
    ...
    if (testButton) {
      ...
      try {
        const data = await api(`/api/proxies/${testButton.dataset.testProxy}/test`, {
          method: 'POST',
          body: {
            destinationHost: profileForm.elements.host.value || 'play.bananasmp.net',
            destinationPort: profileForm.elements.port.value || 25565
          }
        });
        await refreshProxies();
        showToast(`Proxy OK: ${data.result.latencyMs}ms`);
      } catch (error) {
        showToast(error.message);
      } finally {
