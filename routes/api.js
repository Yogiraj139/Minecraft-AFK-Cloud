...
export function apiRoutes({ db, botManager, proxyManager, logger, config }) {
  ...
  router.get('/logs', (req, res) => {
    ok(res, { logs: db.listLogs(Number(req.query.limit || 200)) });
  });
  ...
  router.post('/profiles/:id/duplicate', asyncHandler(async (req, res) => {
    ...
    const copy = db.saveProfile({
      ...original,
      id: undefined,
      name: `${original.name} Copy`,
      minecraftPassword: original.minecraftPassword,
      serverAuthPassword: original.serverAuthPassword
    });
    ok(res, { profile: copy });
  }));

  router.get('/proxies', (req, res) => {
    ok(res, { proxies: db.listProxies() });
  });

  router.post('/proxies/import', asyncHandler(async (req, res) => {
    const result = proxyManager.importList(req.body.proxies);
    logger.info('proxy', `Imported ${result.created.length} proxies`, {
  ...
  router.post('/proxies/:id/test', asyncHandler(async (req, res) => {
    const result = await proxyManager.testProxy(
      ...
      req.body.destinationHost || req.body.host || 'play.bananasmp.net',
      Number(req.body.destinationPort || req.body.port || 25565)
    );
    logger.info('proxy', `Proxy test OK: ${result.id} in ${result.latencyMs}ms`, {
  ...
  router.post('/bot/start', asyncHandler(async (req, res) => {
    ...
    const profile = requestedProfileId
      ? db.getProfile(requestedProfileId)
      : db.getDefaultProfile();
  ...
  router.post('/bot/restart', asyncHandler(async (req, res) => {
    ...
    if (requestedProfileId) {
      ...
      await botManager.start(requestedProfileId, { persist: true, reason: 'dashboard restart' });
    } else {
      const fallbackProfile = db.getDefaultProfile();

      if (!botManager.getState().profileId && !fallbackProfile) {
        throw new Error('Create or select a profile first');
      }

      if (!botManager.getState().profileId && fallbackProfile) {
        await botManager.start(fallbackProfile.id, { persist: true, reason: 'dashboard restart' });
      } else {
  ...
  router.post('/bot/reconnect', asyncHandler(async (req, res) => {
    ...
    if (requestedProfileId) {
      ...
      await botManager.start(requestedProfileId, { persist: true, reason: 'dashboard force reconnect' });
    } else {
      const fallbackProfile = db.getDefaultProfile();
      ...
      if (!botManager.getState().profileId && fallbackProfile) {
        await botManager.start(fallbackProfile.id, { persist: true, reason: 'dashboard force reconnect' });
      } else {
