...
function chatComponentText(value) {
  ...
  if (Array.isArray(value)) {
    return value.map(chatComponentText).filter(Boolean).join(' ');
  }
  ...
  if (typeof value.text === 'string') {
    ...
    parts.push(chatComponentText(value.text.value));
  }

  if (typeof value.translate === 'string') {
    parts.push(value.translate);
  } else if (value.translate?.value) {
    parts.push(chatComponentText(value.translate.value));
  }

  if (value.extra) {
    parts.push(chatComponentText(value.extra));
  }
...
function cleanReason(reason) {
  ...
  const extractedText = chatComponentText(reason);
...
function isGuardDeny(message) {
  return /guard|currently denied|please wait a few minutes/i.test(String(message || ''));
}
...
function parseVersionList(profile, config, pingResponse = null) {
  ...
  if (profile.version && profile.version !== 'auto') {
    return unique([profile.version]).filter(isSupportedVersion);
  }
...
function ensureProfileProxyAvailable(db, profile) {
  ...
  if (profile.proxyMode === 'fixed') {
    ...
    if (!proxy?.enabled) {
      throw new Error('Proxy mode is fixed, but the selected proxy is missing or disabled');
    }

    return;
  }

  const hasEnabledProxy = db.listProxies().some((proxy) => proxy.enabled);

  if (!hasEnabledProxy) {
    throw new Error('Proxy rotation is enabled, but no enabled proxies are imported');
  }
}
...
export class MinecraftBotManager extends EventEmitter {
        ...
        try {
          ...
          if (isGuardDeny(error.message)) {
            ...
            const guardError = new Error(
              rotatingProxy
                ? 'Server GUARD denied entry. Rotating proxy after cooldown.'
                : 'Server GUARD denied entry. Waiting before reconnecting.'
            );
            guardError.retryDelayMs = rotatingProxy ? 90 * 1000 : 5 * 60 * 1000;
            throw guardError;
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      bot.once('end', (reason) => {
        ...
        if (!settled) {
          ...
          const detail = (!endReason || endReason === 'socketClosed') && this.state.lastDisconnectReason
            ? this.state.lastDisconnectReason
            : endReason;
  ...
  attachBotEvents(bot, profile, generation) {
    ...
    bot.on('end', (reason) => {
      ...
      const detail = (!endReason || endReason === 'socketClosed') && this.state.lastDisconnectReason
        ? this.state.lastDisconnectReason
        : endReason || this.state.lastDisconnectReason || 'Disconnected';
      this.state.status = 'offline';
