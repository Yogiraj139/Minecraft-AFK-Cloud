...
function timeoutPromise(ms, label) {
  return new Promise((_, reject) => setTimeout(() => reject(new Error(`${label} timed out after ${ms}ms`)), ms).unref?.());
}
...
function parseProxyLine(line) {
  ...
  if (!/^[a-z][a-z0-9+.-]*:\/\//i.test(normalized)) {
    ...
    if (parts.length === 4) {
      ...
      throw new Error(`Invalid proxy "${trimmed}"`);
    }
  }

  const url = new URL(normalized);
  const type = url.protocol.replace(':', '').toLowerCase();
  if (!['http', 'https', 'socks4', 'socks5'].includes(type)) throw new Error(`Unsupported proxy type "${type}"`);
...
async function waitForSocket(socket, label) {
  await Promise.race([
...
async function connectHttpProxy(proxy, destinationHost, destinationPort) {
  ...
  await waitForSocket(socket, 'Proxy TCP connection');
  ...
  if (!/^HTTP\/1\.[01] 2\d\d/i.test(response)) {
    socket.destroy();
    throw new Error(`Proxy CONNECT failed: ${String(response).split('\r\n')[0] || 'empty response'}`);
  }
...
export class ProxyManager {
  ...
  importList(text) {
    ...
    for (const line of normalizeProxyEntries(text)) {
      try {
        ...
        if (parsed) created.push(this.db.saveProxy(parsed));
      } catch (error) {
  ...
  selectProxy(profile) {
    if (!profile || profile.proxyMode === 'disabled') return null;
    ...
    const proxies = this.db.listProxies({ includeSecrets: true })
      ...
      .sort((a, b) => (a.failureCount || 0) - (b.failureCount || 0) || a.id - b.id);
  ...
  async connect(proxy, destinationHost, destinationPort) {
    ...
    if (proxy.type === 'http' || proxy.type === 'https') return connectHttpProxy(proxy, destinationHost, destinationPort);
  ...
  async testProxy(proxyId, destinationHost = 'play.bananasmp.net', destinationPort = 25565) {
    ...
    try {
      ...
      return { id: proxy.id, ok: true, latencyMs: Date.now() - startedAt, destination: `${destinationHost}:${destinationPort}` };
    } catch (error) {
